        ...
        JavaPairRDD<Integer, Double> newRanks =        
            sites.join(ranks).
                flatMapToPair(kv -> {
                    ...
                    Tuple2<Website, Double> website_currentRank = kv._2();
                    Website website = website_currentRank._1();
                    Double currentRank = website_currentRank._2();
                    Integer destination_count = website.getNEdges(); // number of edges outbound from this site.

                    List<Tuple2<Integer, Double> > contributions = new LinkedList<Tuple2<Integer, Double> >();
                    Iterator<Integer> edgeIterator = website.edgeIterator(); // iterator over edges outbound from this site

                    while(edgeIterator.hasNext()){
                        final int target = edgeIterator.next();
                        contributions.add(new Tuple2(target, currentRank / destination_count));
                    }
