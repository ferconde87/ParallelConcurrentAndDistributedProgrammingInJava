...
public class MatrixMult {
    ...
    public static void parallelMatrixMultiply(Matrix a, Matrix b, Matrix c,
        ...
        final int myrank = mpi.MPI_Comm_rank(mpi.MPI_COMM_WORLD);//rank of the current process
        final int size = mpi.MPI_Comm_size(mpi.MPI_COMM_WORLD);//number of ranks that we are working
        final int nrows = c.getNRows();
        final int rowChunk = (nrows + size - 1) / size;//chunk of rows to separate 'rowChunk' rows per process
        int startRow = myrank * rowChunk;//start row for the current rank
        int endRow = (myrank + 1) * rowChunk;//end row for the current rank
        if (endRow > nrows) endRow = nrows;

        // For simplicity, broadcast all of matrices a & b to all the ranks
        //A more efficient approach will be to only send the relevant rows/columns to the proper rank
        ...
        if(myrank == 0){
            ...
            for(int i = 1; i < size; ++i){
                ...
                request[i - 1] = mpi.MPI_Irecv(c.getValues(), rowOffSet, nElements, i, i, mpi.MPI_COMM_WORLD);
            }
